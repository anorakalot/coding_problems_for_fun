...
procedure Tic_Tac_Toe_Program is
   ...
   valid_check_all_spaces_bool : valid_bool := 0;
   keep_game_going_bool : valid_bool := 1;
   replay_game_bool : valid_bool := 0;
...
begin
   ...
   while (keep_game_going_bool = 1) loop
   --display_board;
      case game_state is --transitions
         ...
         when GAME_OVER =>
            ...
            --if (replay_game_bool = 0) then
            if (char_input = 'Y' or char_input='y')then
               Put_Line("inside play again");
               game_state := GAME_INIT;
               keep_game_going_bool :=1;
               board := ((NA, NA, NA), (NA, NA, NA), (NA, NA, NA));
            else
            game_state :=
                 GAME_OVER;
               keep_game_going_bool :=0;
               --might need to change this looping back to game over state
            end if;
               --elsif (replay_game_bool = 1) then
            --game_state:=
            --need to add a refurbish state to set board back to normal or just do it in this state
            --  end case
      end case;
      ...
      case game_state is --actions
         ...
         when CHOOSE_X_O =>
            ...
            if (char_input = 'X') then
               ...
               Put_Line
                 ("Say not a valid input will just make you start with the X character");
               input := X;
            end if;
         ...
         when CHECK_IF_VALID_MOVE =>
            ...
            if board (y_input, x_input) /= X and board (y_input, x_input) /= O
            then
               Put_Line ("input is valid!");
               valid_move_bool := 1;
               --so it transitions to the next state correctly
         ...
         when CHECK_WIN =>
            ...
            for y in y_col_index loop
               ...
               if (check_win_cond_var = NA) then
                  ...
                  valid_row_win_bool   := 0;
                  Put_Line ("Row_check_bool is 0 for this row");
               ...
               if valid_row_win_bool = 1 then
                  ...
                  Put_Line (X_OR_O'Image(input) & " won!");
                  valid_win_bool := 1;
               end if;
            end loop;
            ...
            if (valid_win_bool /= 1)
            then --meaning only if not found win in row
            ...
            --if (apple = 1)then --just to "commment" out this block of code
            Put_Line ("checking diagonal win condition down to the right ");
      ...
      --just like col check above only go if valid_win_bool not already found
            ...
            --unlike in for loop need to make sure y and x are set to 0 first;
              ...
              Put_Line ("checking diagonal win condition down to the left ");
            if (valid_win_bool /= 1) then
               valid_diag_check_bool := 1;
               valid_diag_win_bool   := 1;
               check_win_cond_var    := board (0, 2);
               if (check_win_cond_var = NA) then
                  valid_diag_check_bool := 0;
                  valid_diag_win_bool   := 0;
               --if first value is NA then this row automatically isn't valid
      ...
      --just like col check above only go if valid_win_bool not already found
               ...
               x_int := 2;
            --unlike in for loop need to make sure y and x are set to 0 first;
               while (y_int < 3 and x_int >= 0) loop
                  y_input := y_col_index (y_int);
                  x_input := x_row_index (x_int);
                  if (valid_diag_check_bool = 1) and
                    (board (y_input, x_input) /= check_win_cond_var)
               ...
               if valid_diag_win_bool = 1 then
                  Put_Line ("found winning combination in diag_check_2");
                  Put_Line (X_OR_O'Image(input) & " won!");
                  valid_win_bool := 1;
               end if;
            ...
            --need to check if all spaces are taken and no winner
            if (valid_win_bool = 0) then
               valid_check_all_spaces_bool :=1;

               for y in y_col_index loop
                  for x in x_row_index loop
                     if (board(y,x) = NA) then
                        valid_check_all_spaces_bool :=0;
                     end if;
                     end loop;
                  end loop;

                  if (valid_check_all_spaces_bool = 1)then
                     valid_win_bool :=1;
                     Put_Line("Nobody won");
                  end if;
         ...
         when SWITCH_PLAYER =>
            if (input = X) then
               Put_Line ("Switching to O now");
               input := O;
            elsif (input = O) then
               Put_Line ("Switching to X now");
               input := X;
            end if;
         when GAME_OVER =>
            Put_Line ("Game_Over!");
