...
package Animal is
   ...
   height_too_large : exception;
   type Creature is new Ada.Finalization.Controlled with private;
   -- this is the constructor where one can specify all of
   -- the inputs.
   function Init
     (Name       : in String; Legs : in Natural; WeightInGrams : in Positive;
      HeightInCm : in Positive) return Creature;
   -- this is the constructor where all of the inputs are
   -- defaults. Also known as the default constructor.
   ...
   procedure Set_Height (Creat : in out Creature; HeightInCm : in Positive);
   ...
   function get_name (creat : in out creature) return Ada.Strings.Unbounded.Unbounded_string;
private
   ...
   -- a private version of the procedure that will do the
   -- actual printing.
   procedure Private_Print_Record (Creat : in out Creature);
   procedure Set_Legs_private (Creat : in out Creature; Legs : in Natural);
   procedure set_height_private( creat : in out Creature; heightincm : in positive);
