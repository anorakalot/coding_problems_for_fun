...
int main(){
    ...
    //printf("DISTINCT ARRAY level\n");
    int last_value= -98128;
    std::vector<int> distinct_array;
    for (int i = 0; i < n; i++){
        if (last_value==-98128){
            last_value= array[i];
            //printf("Setting initial last_value to %i \n", last_value);
            distinct_array.push_back(last_value);//pushes back last_value initial
            continue;
        }
        if(array[i] == last_value){
            //does not push_back
            continue; // continues to next value since not distinct
        }
        else{
            //printf("Pushing back distinct value %i \n", last_value);
            distinct_array.push_back(array[i]);
            last_value = array[i];
        }
